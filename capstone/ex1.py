...
def main_application():
    ...
    print(f"메인 스크립트 실행 중... (종료 버튼: GPIO {STOP_BUTTON_PIN})")

    try:
        while True:
            # 버튼이 눌렸는지 폴링 방식으로 반복 체크
            if GPIO.input(STOP_BUTTON_PIN) == GPIO.LOW:
                print("\n[알림] 정지 버튼 감지! 프로그램을 안전하게 종료합니다.")
                break

            # 아래에 메인 코드(예: 소리 분석, 디스플레이 등) 삽입
                
            print(".", end='', flush=True)
            time.sleep(1)
    except KeyboardInterrupt:
        print("\nCtrl+C로 프로그램을 강제 종료합니다.")
    finally:
...
try:
    ...
    while True:
        ...
        # 3채널 버퍼를 float으로 변환하여 RMS 계산 준비
