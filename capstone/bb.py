...
DIRECTION_LABELS = ["오른쪽", "왼쪽"]  # 채널 0, 1, 2에 해당하는 방향
...
def display_result_text(result_text):
    img = master_img.copy()  # 마스터 이미지를 항상 새로 복사
    draw = ImageDraw.Draw(img)
    font = ImageFont.load_default()
    bbox = draw.textbbox((0,0), result_text, font=font)
    x = (WIDTH-(bbox[2]-bbox[0]))//2
    y = TOP_BANNER_HEIGHT + (HEIGHT-TOP_BANNER_HEIGHT-(bbox[3]-bbox[1]))//2
    draw.text((x, y), result_text, font=font, fill=(255,255,255))
    disp.display(img)
...
def main_application():
    ...
    while True:
        ...
        if idx in sound_category.keys() and confidence >= 0.4:
            ...
            #disp.display(img)
            logo = Image.open("/home/pi/capstone/bgi.jpg").resize((WIDTH, TOP_BANNER_HEIGHT), Image.LANCZOS)
            img.paste(logo, (0, 0))
            text = most_common
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            #draw.rectangle((0, 0, WIDTH, HEIGHT), outline=0, fill=(0, 0, 0))
            # 1. 상단 고정 마스터 이미지 생성
            master_img = Image.new('RGB', (WIDTH, HEIGHT), (0, 0, 0))
            draw_master = ImageDraw.Draw(master_img)
